 ...
 */
export function layoutProductName(value, softLimit = 24) {
    ...
    if (normalized.length <= softLimit)
        return { text: normalized, lines: [normalized], className: classForLength(normalized.length), autoWrapped: false };
    const candidates = [];
    let depth = 0;
    for (let i = 0; i < normalized.length - 1; i++) {
        ...
        if (depth !== 0)
            continue;
        if (BREAK_CHARS.has(ch) || CLOSERS.has(ch))
            candidates.push(i + 1);
    }
