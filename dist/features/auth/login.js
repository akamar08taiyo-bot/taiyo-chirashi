...
export function renderLogin(root, onSuccess) {
    ...
    const idInput = root.querySelector('#employee-id');
    const passInput = root.querySelector('#password');
    const errorEl = root.querySelector('#login-error');
    root.querySelectorAll('[data-demo-id]').forEach((button) => button.addEventListener('click', () => { if (idInput)
        idInput.value = button.dataset.demoId ?? ''; if (passInput)
        passInput.value = localDemoLoginInfo.password; idInput?.focus(); }));
    form?.addEventListener('submit', async (e) => { e.preventDefault(); if (!idInput || !passInput || !errorEl)
        return; errorEl.textContent = ''; const submit = form.querySelector('button[type=submit]'); if (submit) {
        submit.disabled = true;
        submit.textContent = 'ログイン中…';
    } try {
        const session = await login(idInput.value, passInput.value);
        showToast('ログインしました', 'success');
        await onSuccess(session);
    }
    catch (error) {
        errorEl.textContent = error instanceof Error ? error.message : 'ログインできませんでした。';
    }
    finally {
        if (submit) {
