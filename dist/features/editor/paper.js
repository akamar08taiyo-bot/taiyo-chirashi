...
function renderWelfareCard(record, index, selectedIndex, interactive) {
    ...
    const productMain = `<div class="case-product-main"${editableAttrs('item-product', 50, interactive)}>${escapeHtml(item.productName)}</div>`;
    const codeHtml = item.productCode ? `<span class="equip-code">品番 <span${editableAttrs('item-code', 30, interactive)}>${escapeHtml(item.productCode)}</span></span>` : '';
    const badge = item.equipmentCategory ? `<div class="equip-badge">${escapeHtml(item.equipmentCategory)}</div>` : '';
    const metaParts = [item.maker ? escapeHtml(item.maker) : '', item.taisCode ? `TAIS ${escapeHtml(item.taisCode)}` : '', codeHtml].filter(Boolean);
    const meta = metaParts.length ? `<div class="equip-meta">${metaParts.join('　')}</div>` : '';
    const freeTag = item.assistBarFree ? `<div class="free-tag">${escapeHtml(item.assistBarLabel || '介助バー無料')}</div>` : '';
    return `<article class="case-card${index === selectedIndex ? ' selected' : ''}" data-item-index="${index}" ${interactive ? 'draggable="true"' : ''}><div class="case-photo" data-preview-action="photo" title="写真をクリックして調整">${photoHtml(record, index)}</div><div class="case-copy">${badge}${productMain}<div class="case-title"${editableAttrs('item-title', 40, interactive)}>${escapeHtml(item.title)}</div><div class="case-desc${descriptionClass}"${editableAttrs('item-description', 180, interactive)}>${escapeHtml(item.description)}</div>${meta}${freeTag}${state.display.showUnits ? `<div class="unit-line"><span>単位数</span><span><strong${editableAttrs('item-units', 8, interactive)}>${Math.max(0, item.units).toLocaleString('ja-JP')}</strong> 単位／月</span></div>` : ''}<div class="burden-line" data-preview-action="cost" title="料金は右側の基本情報で編集">${burdens}</div></div></article>`;
}
