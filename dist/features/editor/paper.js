...
function editableAttrs(field, maxLength, interactive) {
...
export function renderPaper(record, context, selectedIndex = -1, interactive = true) {
    ...
    <div class="company-block">${showLogo && context.organization.logoUrl ? `<img class="company-logo" src="${escapeAttr(context.organization.logoUrl)}" alt="会社ロゴ">` : ''}<strong>${escapeHtml(context.organization.name)}</strong><span>${escapeHtml(office?.name ?? '')}</span>${office?.address ? `<span class="company-address">${escapeHtml(office.address)}</span>` : ''}<span>TEL:${escapeHtml(office?.phone ?? '')}</span><span>FAX:${escapeHtml(office?.fax ?? '')}</span>${contactName || mobilePhone ? `<span>担当：${escapeHtml(contactName)}${mobilePhone ? ` ${escapeHtml(mobilePhone)}` : ''}</span>` : ''}</div></header>
    ...
    <footer class="flyer-footer"><div><strong${editableAttrs('footer-headline', 100, interactive)}>${escapeHtml(state.footerHeadline)}</strong><small${editableAttrs('footer-note', 220, interactive)}>${escapeHtml(state.footerNote)}</small></div><div class="footer-contact"><strong>${escapeHtml(context.organization.name)} ${escapeHtml(office?.name ?? '')}</strong><span>TEL:${escapeHtml(office?.phone ?? '')} / FAX:${escapeHtml(office?.fax ?? '')}</span></div></footer>
  </article>`;
}
function renderCard(record, index, selectedIndex, interactive) {
...
function photoHtml(record, index) {
    const item = record.editorState.items[index];
    if (!item)
        return '';
    const fitMode = item.transform.fitMode ?? 'cover';
...
function renderWelfareCard(record, index, selectedIndex, interactive) {
    const state = record.editorState;
    const item = state.items[index];
    if (!item)
        return '';
    const burden = calculateBurdenAmounts(item.monthlyAmount);
    ...
    return `<article class="case-card${index === selectedIndex ? ' selected' : ''}" data-item-index="${index}" ${interactive ? 'draggable="true"' : ''}><div class="case-photo" data-preview-action="photo" title="写真をクリックして調整">${photoHtml(record, index)}</div><div class="case-copy"><div class="case-title"${editableAttrs('item-title', 40, interactive)}>${escapeHtml(item.title)}</div><div class="case-desc${descriptionClass}"${editableAttrs('item-description', 180, interactive)}>${escapeHtml(item.description)}</div>${product}${state.display.showUnits ? `<div class="unit-line"><span>単位数</span><span><strong${editableAttrs('item-units', 8, interactive)}>${Math.max(0, item.units).toLocaleString('ja-JP')}</strong> 単位／月</span></div>` : ''}<div class="burden-line" data-preview-action="cost" title="料金は右側の基本情報で編集">${burdens}</div></div></article>`;
}
function renderConsumableCard(record, index, selectedIndex, interactive) {
    const state = record.editorState;
    const item = state.items[index];
    if (!item)
        return '';
...
export function findOverflowIndexes(paper) {
    const result = [];
    paper.querySelectorAll('.case-card').forEach((card) => {
        const index = Number(card.dataset.itemIndex);
        const copy = card.querySelector('.case-copy');
        const desc = card.querySelector('.case-desc');
        const product = card.querySelector('.consumable-product-name');
        if ((copy && copy.scrollHeight > copy.clientHeight + 2) || (desc && desc.scrollHeight > desc.clientHeight + 2) || (product && (product.scrollHeight > product.clientHeight + 2 || product.scrollWidth > product.clientWidth + 2)))
            result.push(index);
    });
    return result;
}
