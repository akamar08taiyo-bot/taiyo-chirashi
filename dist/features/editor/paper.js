...
function renderWelfareCard(record, index, selectedIndex, interactive) {
    ...
    return `<article class="case-card${index === selectedIndex ? ' selected' : ''}" data-item-index="${index}" ${interactive ? 'draggable="true"' : ''}><div class="case-photo" data-preview-action="photo" title="写真をクリックして調整">${photoHtml(record, index)}</div><div class="case-copy">${badge}<div class="case-title"${editableAttrs('item-title', 40, interactive)}>${escapeHtml(item.title)}</div><div class="case-desc${descriptionClass}"${editableAttrs('item-description', 180, interactive)}>${escapeHtml(item.description)}</div>${product}${meta}${freeTag}${state.display.showUnits ? `<div class="unit-line"><span>単位数</span><span><strong${editableAttrs('item-units', 8, interactive)}>${Math.max(0, item.units).toLocaleString('ja-JP')}</strong> 単位／月</span></div>` : ''}<div class="burden-line" data-preview-action="cost" title="料金は右側の基本情報で編集">${burdens}</div></div></article>`;
}
