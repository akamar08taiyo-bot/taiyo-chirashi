...
export async function ensureLocalSeeds() {
    if (!isLocalMode)
        return;
...
export async function listFlyers(session, includeDeleted = false) {
    if (isLocalMode) {
        ...
        const all = (await kvGet(LOCAL_FLYERS)) ?? [];
    ...
    const rows = await supabaseRequest(`/rest/v1/flyers?select=*&${deletedFilter}&order=updated_at.desc`, { token: session.accessToken });
    return rows.map(mapFlyer);
}
export async function getFlyer(session, id) {
    if (isLocalMode)
        return (await listAllLocal()).find((record) => record.id === id && canReadLocal(record, session)) ?? null;
    const rows = await supabaseRequest(`/rest/v1/flyers?id=eq.${encodeURIComponent(id)}&select=*`, { token: session.accessToken });
    return rows[0] ? mapFlyer(rows[0]) : null;
}
export async function createFlyer(session, context, input) {
    ...
    if (isLocalMode) {
        ...
        all.unshift(base);
        await kvSet(LOCAL_FLYERS, all);
        return structuredClone(base);
    }
    const payload = toInsertPayload(base);
    delete payload.id;
    const rows = await supabaseRequest('/rest/v1/flyers', { method: 'POST', token: session.accessToken, body: payload, prefer: 'return=representation' });
    const row = rows[0];
    if (!row)
        throw new AppError('新しい事例集を作成できませんでした。もう一度お試しください。');
    return mapFlyer(row);
}
export async function saveFlyer(session, record, expectedVersion) {
    if (isLocalMode) {
        const all = await listAllLocal();
        const index = all.findIndex((entry) => entry.id === record.id);
        if (index < 0)
            return { ok: false, record: null, conflict: false, errorMessage: '保存先が見つかりませんでした。' };
        const current = all[index];
        if (!current)
            return { ok: false, record: null, conflict: false, errorMessage: '保存先が見つかりませんでした。' };
        if (current.version !== expectedVersion)
            return { ok: false, record: structuredClone(current), conflict: true, errorMessage: null };
        const saved = structuredClone(record);
        saved.version = expectedVersion + 1;
        saved.updatedAt = new Date().toISOString();
        all[index] = saved;
        await kvSet(LOCAL_FLYERS, all);
        return { ok: true, record: structuredClone(saved), conflict: false, errorMessage: null };
    }
    try {
        const rows = await supabaseRequest('/rest/v1/rpc/update_flyer_versioned', {
        ...
        const row = rows[0];
        if (!row)
            return { ok: false, record: await getFlyer(session, record.id), conflict: true, errorMessage: null };
...
export async function duplicateFlyer(session, source, titleSuffix = '（コピー）') {
    const now = new Date().toISOString();
    const copy = { ...structuredClone(source), id: createId(), ownerId: session.profile.id, assigneeId: session.profile.id, officeId: session.profile.officeId, shareScope: 'private', title: `${source.title}${titleSuffix}`, version: 1, createdAt: now, updatedAt: now, deletedAt: null };
    ...
    const payload = toInsertPayload(copy);
    delete payload.id;
    const rows = await supabaseRequest('/rest/v1/flyers', { method: 'POST', token: session.accessToken, body: payload, prefer: 'return=representation' });
    const row = rows[0];
    if (!row)
        throw new AppError('複製できませんでした。もう一度お試しください。');
    return mapFlyer(row);
}
export async function setFlyerDeleted(session, id, deleted) {
    if (isLocalMode) {
        const all = await listAllLocal();
        ...
        if (!current)
            return;
        if (!canDeleteLocal(current, session))
            throw new AppError('この作品を削除・復元する権限がありません。');
        all[index] = { ...current, deletedAt: deleted ? new Date().toISOString() : null, updatedAt: new Date().toISOString() };
        await kvSet(LOCAL_FLYERS, all);
        return;
    }
    const rows = await supabaseRequest('/rest/v1/rpc/set_flyer_deleted', { method: 'POST', token: session.accessToken, body: { p_flyer_id: id, p_deleted: deleted } });
    if (!rows.length)
        throw new AppError('この作品を削除・復元する権限がありません。');
}
export async function permanentlyDeleteFlyer(session, id) {
    if (isLocalMode) {
        const all = await listAllLocal();
        const record = all.find(r => r.id === id);
        if (!record?.deletedAt || Date.now() - new Date(record.deletedAt).getTime() < 30 * 86400000)
            throw new AppError('削除から30日経過するまで完全削除できません。');
        if (!canDeleteLocal(record, session))
            throw new AppError('完全削除する権限がありません。');
        await kvSet(LOCAL_FLYERS, all.filter((record) => record.id !== id));
        return;
    }
...
async function listAllLocal() { await ensureLocalSeeds(); return (await kvGet(LOCAL_FLYERS)) ?? []; }
async function getTemplateById(session, id) {
    if (isLocalMode) {
        await ensureLocalSeeds();
        return ((await kvGet(LOCAL_TEMPLATES)) ?? []).find((t) => t.id === id) ?? null;
    }
    const rows = await supabaseRequest(`/rest/v1/templates?id=eq.${encodeURIComponent(id)}&deleted_at=is.null&select=*`, { token: session.accessToken });
    const row = rows[0];
    return row ? { id: row.id, organizationId: row.organization_id, officeId: row.office_id, ownerId: row.owner_id, name: row.name, categoryId: row.category_id, shareScope: row.share_scope, editorState: normalizeEditorState(row.editor_state), createdAt: row.created_at, updatedAt: row.updated_at, deletedAt: row.deleted_at } : null;
}
function canReadLocal(record, session) {
    const p = session.profile;
    if (record.ownerId === p.id)
        return true;
    if (record.organizationId !== p.organizationId)
        return false;
    if (p.role === 'org_admin' && record.shareScope !== 'private')
        return true;
    if (record.shareScope === 'company')
        return true;
    return record.shareScope === 'office' && record.organizationId === p.organizationId && record.officeId === p.officeId;
}
function mapFlyer(row) { return { id: row.id, organizationId: row.organization_id, officeId: row.office_id, ownerId: row.owner_id, assigneeId: row.assignee_id, title: row.title, categoryId: row.category_id, shareScope: row.share_scope, orientation: row.orientation, layoutCount: row.layout_count, designStyle: row.design_style, mainColor: row.main_color, editorState: normalizeEditorState(row.editor_state), version: row.version, createdAt: row.created_at, updatedAt: row.updated_at, deletedAt: row.deleted_at }; }
function toInsertPayload(record) { return { id: record.id, organization_id: record.organizationId, office_id: record.officeId, owner_id: record.ownerId, assignee_id: record.assigneeId, title: record.title, category_id: record.categoryId, share_scope: record.shareScope, orientation: record.orientation, layout_count: record.layoutCount, design_style: record.designStyle, main_color: record.mainColor, editor_state: sanitizeEditorStateForServer(record.editorState), version: record.version }; }
function canDeleteLocal(record, session) { const p = session.profile; if (record.ownerId === p.id)
    return true; if (record.organizationId !== p.organizationId || record.shareScope === 'private')
    return false; if (p.role === 'org_admin')
    return true; return p.role === 'office_admin' && record.officeId === p.officeId; }
//# sourceMappingURL=flyerService.js.map