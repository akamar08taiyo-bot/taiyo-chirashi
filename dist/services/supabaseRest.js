...
export async function supabaseRequest(path, options = {}) {
    ...
    try {
        ...
        if (error instanceof DOMException && error.name === 'AbortError')
            throw new AppError('通信に時間がかかっています。通信状況を確認して、もう一度お試しください。');
...
export async function supabaseUpload(path, blob, token) {
    return supabaseUploadToBucket('flyer-media', path, blob, token, false);
}
export async function supabaseUploadToBucket(bucket, path, blob, token, upsert = false) {
    const controller = new AbortController();
    ...
    try {
        ...
        if (!response.ok)
            throw new AppError('画像をアップロードできませんでした。元の画像は削除されていません。', response.status);
    }
    catch (error) {
        if (error instanceof AppError)
            throw error;
...
export async function storageSignedUrl(bucket, path, token, expiresIn = 3600) {
    if (!path)
        return '';
    const result = await supabaseRequest(`/storage/v1/object/sign/${encodeURIComponent(bucket)}/${encodePath(path)}`, { method: 'POST', token, body: { expiresIn } });
...
function mapHttpMessage(status, fallback) {
    if (status === 401)
        return 'ログインの有効期限が切れました。もう一度ログインしてください。編集中の内容は一時保存されています。';
    if (status === 403)
        return 'この操作を行う権限がありません。';
    if (status === 404)
        return '対象のデータが見つかりませんでした。';
    if (status === 409)
        return '別の更新と重なりました。最新状態を確認してください。';
    if (status >= 500)
        return 'サーバーで一時的な問題が発生しました。少し待ってからもう一度お試しください。';
