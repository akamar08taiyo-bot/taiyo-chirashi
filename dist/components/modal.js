...
export function showModal(options) {
    return new Promise((resolve) => {
        ...
        overlay.addEventListener('click', (e) => { if (e.target === overlay && options.dismissible !== false)
            close(null); });
        overlay.querySelectorAll('[data-modal-value]').forEach((button) => button.addEventListener('click', () => close(button.dataset.modalValue ?? null)));
        document.body.append(overlay);
        overlay.querySelector('input,textarea,button')?.focus();
    });
}
export function promptText(title, label, initial = '', submitLabel = '決定', multiline = true) {
    return new Promise((resolve) => { const overlay = document.createElement('div'); overlay.className = 'modal-overlay'; overlay.innerHTML = `<section class="modal" role="dialog" aria-modal="true"><header><h2>${escapeHtml(title)}</h2><button class="icon-btn close">${icon('close')}</button></header><div class="modal-body"><label class="modal-field">${escapeHtml(label)}${multiline ? `<textarea id="modal-prompt-text" rows="5"></textarea>` : `<input type="text" id="modal-prompt-text" maxlength="60">`}</label></div><footer><button class="btn secondary cancel">キャンセル</button><button class="btn primary submit">${escapeHtml(submitLabel)}</button></footer></section>`; const area = overlay.querySelector('#modal-prompt-text'); if (area)
        area.value = initial; const done = (v) => { overlay.remove(); resolve(v); }; overlay.querySelector('.close')?.addEventListener('click', () => done(null)); overlay.querySelector('.cancel')?.addEventListener('click', () => done(null)); overlay.querySelector('.submit')?.addEventListener('click', () => done(area?.value.trim() || '')); if (!multiline)
        area?.addEventListener('keydown', (e) => { if (e.key === 'Enter') {
            e.preventDefault();
            done(area.value.trim());
        } }); document.body.append(overlay); area?.focus(); });
}
