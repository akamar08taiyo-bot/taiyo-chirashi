...
import { navigate, startRouter } from './app/router.js';
...
async function bootstrap() {
    ...
    if (appState.session) {
        try {
            appState.context = await loadAppContext(appState.session);
        }
        catch {
...
async function renderRoute(route) {
    ...
    cleanup = null;
    if (!appState.session) {
        renderLogin(root, async (session) => { appState.session = session; appState.context = await loadAppContext(session); navigate('home'); });
        return;
    }
    if (!appState.context) {
        appState.context = await loadAppContext(appState.session);
    ...
    if (route.name === 'home')
        await renderHome(root, session, context);
    else if (route.name === 'create')
        await renderCreateWizard(root, session, context);
    else if (route.name === 'editor' && route.params.id)
        cleanup = await renderEditor(root, route.params.id, { session, context });
    else if (route.name === 'templates')
        await renderTemplates(root, session, context);
    else if (route.name === 'media')
        await renderMediaLibrary(root, session, context);
    else if (route.name === 'admin')
        await renderAdmin(root, session, context);
    else if (route.name === 'trash')
        await renderTrash(root, session, context);
    else
        navigate('home');
}
...
function bindGlobalEvents() {
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (profileButton) {
            e.stopPropagation();
            root.querySelector('#profile-popover')?.classList.toggle('open');
            return;
        }
