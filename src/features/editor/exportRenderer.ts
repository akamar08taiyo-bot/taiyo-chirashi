import type { AppContext, FlyerRecord, FlyerItem } from '../../types.js';
...
export async function renderFlyerCanvas(record:FlyerRecord,context:AppContext):Promise<HTMLCanvasElement>{
...
export async function findUnavailableExportImages(record:FlyerRecord):Promise<number[]>{
  const failed:number[]=[];
  for(let i=0;i<record.editorState.layoutCount;i++){
    const media=record.editorState.items[i]?.media;if(!media)continue;
    try{const local=await getOriginalBlobForRef(media);const source=local??media.originalUrl??media.previewUrl;if(!source)throw new Error('missing source');await loadImage(source);}catch{failed.push(i);}
  }
  return failed;
}

export async function exportPng(record:FlyerRecord,context:AppContext):Promise<void>{const canvas=await renderFlyerCanvas(record,context);const blob=await canvasBlob(canvas,'image/png');download(blob,`${safeName(record.title)}.png`);}

export async function printPng(record:FlyerRecord,context:AppContext):Promise<void>{
  const canvas=await renderFlyerCanvas(record,context);const blob=await canvasBlob(canvas,'image/png');await printPngBlob(blob,record.orientation,record.title);
}
export async function exportJpeg(record:FlyerRecord,context:AppContext):Promise<void>{const canvas=await renderFlyerCanvas(record,context);const blob=await canvasBlob(canvas,'image/jpeg',0.94);download(blob,`${safeName(record.title)}.jpg`);}
export async function exportPdf(record:FlyerRecord,context:AppContext):Promise<void>{const canvas=await renderFlyerCanvas(record,context);const jpeg=await canvasBlob(canvas,'image/jpeg',0.95);const bytes=new Uint8Array(await jpeg.arrayBuffer());const pdf=buildSingleImagePdf(bytes,canvas.width,canvas.height,record.orientation);download(new Blob([pdf],{type:'application/pdf'}),`${safeName(record.title)}.pdf`);}

async function drawCard(ctx:CanvasRenderingContext2D,item:FlyerItem,x:number,y:number,w:number,h:number,state:FlyerRecord['editorState'],accent:string){
  if(state.mode==='consumables')return drawConsumableCard(ctx,item,x,y,w,h,state,accent);
...
async function drawCardPhoto(ctx:CanvasRenderingContext2D,item:FlyerItem,x:number,y:number,w:number,h:number,state:FlyerRecord['editorState'],accent:string){
  ...
  if(item.media){
    try{
      const local=await getOriginalBlobForRef(item.media);const source=local??item.media.originalUrl??item.media.previewUrl;
      if(source){const img=await loadImage(source);drawFittedImage(ctx,img,x+border,y+border,w-border*2,photoH-border,item.transform);}
...
async function drawConsumableCard(ctx:CanvasRenderingContext2D,item:FlyerItem,x:number,y:number,w:number,h:number,state:FlyerRecord['editorState'],accent:string){
  ...
  for(const line of descLines){drawText(ctx,line,x+pad,ty,descSize,'normal','#4a413a');ty+=descSize*1.32;}

  if(presentation.showPrice){
    const lineY=y+h-pad-priceSize*1.05;
    ctx.strokeStyle=lighten(accent,.82);ctx.lineWidth=Math.max(1,w/900);ctx.beginPath();ctx.moveTo(x+pad,lineY-priceSize*.20);ctx.lineTo(x+w-pad,lineY-priceSize*.20);ctx.stroke();
...
function fitTextSize(ctx:CanvasRenderingContext2D,text:string,maxWidth:number,preferred:number,minimum:number,weight:string,family='sans-serif'){
  let size=preferred;while(size>minimum&&measureText(ctx,text,size,weight,family)>maxWidth)size-=1;return Math.max(minimum,size);
}
function drawFittedSingleLine(ctx:CanvasRenderingContext2D,text:string,x:number,y:number,maxWidth:number,preferred:number,minimum:number,weight:string,color:string){
  ...
  if(ctx.measureText(visible).width>maxWidth){
    while(visible.length>1&&ctx.measureText(`${visible}…`).width>maxWidth)visible=visible.slice(0,-1);
    if(visible!==text)visible=`${visible.trimEnd()}…`;
  }
  drawText(ctx,visible,x,y,size,weight,color);
}
function drawFittedImage(ctx:CanvasRenderingContext2D,img:HTMLImageElement,x:number,y:number,w:number,h:number,t:FlyerItem['transform']){ctx.save();ctx.beginPath();ctx.rect(x,y,w,h);ctx.clip();ctx.translate(x+w/2,y+h/2);ctx.rotate(t.rotation*Math.PI/180);const placement=calculateImagePlacement(img.naturalWidth,img.naturalHeight,w,h,t);ctx.drawImage(img,-placement.width/2-placement.offsetX,-placement.height/2-placement.offsetY,placement.width,placement.height);ctx.restore();}
function drawText(ctx:CanvasRenderingContext2D,text:string,x:number,y:number,size:number,weight:string,color:string,family='sans-serif',align:CanvasTextAlign='left',maxWidth?:number){setCanvasFont(ctx,size,weight,family);ctx.fillStyle=color;ctx.textAlign=align;ctx.fillText(text,x+(align==='right'&&maxWidth?maxWidth:0),y,maxWidth);}
function wrapLines(ctx:CanvasRenderingContext2D,text:string,maxWidth:number,size:number,weight:string,family:string,maxLines:number){if(maxLines<=0||!text)return[];setCanvasFont(ctx,size,weight,family);const lines:string[]=[];let line='';for(const ch of text){const next=line+ch;if(ctx.measureText(next).width>maxWidth&&line){lines.push(line);line=ch;if(lines.length>=maxLines)break;}else line=next;}if(lines.length<maxLines&&line)lines.push(line);if(lines.length===maxLines&&text.length>lines.join('').length){let last=lines[maxLines-1]??'';while(last&&ctx.measureText(last+'…').width>maxWidth)last=last.slice(0,-1);lines[maxLines-1]=last+'…';}return lines;}
function gridSpec(count:number){if(count===1)return{cols:1,rows:1};if(count===2)return{cols:2,rows:1};if(count===3)return{cols:3,rows:1};if(count===4)return{cols:2,rows:2};if(count===6)return{cols:3,rows:2};return{cols:3,rows:3};}
function clamp(n:number,min:number,max:number){return Math.max(min,Math.min(max,n));}
function lighten(hex:string,amount:number){const clean=hex.replace('#','');if(clean.length!==6)return'#d9c3ad';const parts=[0,2,4].map((i)=>parseInt(clean.slice(i,i+2),16));return`rgb(${parts.map((p)=>Math.round(p+(255-p)*amount)).join(',')})`;}
function canvasBlob(canvas:HTMLCanvasElement,type:string,quality?:number){return new Promise<Blob>((resolve,reject)=>canvas.toBlob((b)=>b?resolve(b):reject(new AppError('画像データを作成できませんでした。もう一度お試しください。')),type,quality));}
function safeName(name:string){return(name.trim()||'チラシ').replace(/[\\/:*?"<>|]/g,'_').slice(0,80);}
function download(blob:Blob,name:string){const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000);}
async function printPngBlob(blob:Blob,orientation:'portrait'|'landscape',title:string):Promise<void>{
  const imageUrl=URL.createObjectURL(blob);const frame=document.createElement('iframe');frame.setAttribute('aria-hidden','true');frame.tabIndex=-1;frame.style.cssText='position:fixed;left:-10000px;top:0;width:1px;height:1px;border:0;opacity:0;pointer-events:none';document.body.append(frame);
  const cleanup=()=>{URL.revokeObjectURL(imageUrl);frame.remove();};
  try{
    const doc=frame.contentDocument;if(!doc)throw new Error('印刷画面を準備できませんでした。');
    doc.open();doc.write(buildPngPrintDocument(imageUrl,orientation,title));doc.close();
    const image=doc.querySelector<HTMLImageElement>('.print-image');if(!image)throw new Error('PNG印刷用の画像を準備できませんでした。');
    await new Promise<void>((resolve,reject)=>{if(image.complete&&image.naturalWidth>0){resolve();return;}image.addEventListener('load',()=>resolve(),{once:true});image.addEventListener('error',()=>reject(new Error('PNG印刷用の画像を読み込めませんでした。')),{once:true});});
    const win=frame.contentWindow;if(!win)throw new Error('印刷画面を開けませんでした。');
    let cleaned=false;const safeCleanup=()=>{if(cleaned)return;cleaned=true;cleanup();};win.addEventListener('afterprint',safeCleanup,{once:true});win.focus();win.print();window.setTimeout(safeCleanup,30000);
  }catch(error){cleanup();throw error;}
}
function buildSingleImagePdf(jpeg:Uint8Array,pixelW:number,pixelH:number,orientation:'portrait'|'landscape'):Uint8Array<ArrayBuffer>{
  const pageW=orientation==='landscape'?841.89:595.28,pageH=orientation==='landscape'?595.28:841.89;const enc=new TextEncoder();const chunks:Uint8Array[]=[];const offsets:number[]=[0];let len=0;const push=(data:string|Uint8Array)=>{const b=typeof data==='string'?enc.encode(data):data;chunks.push(b);len+=b.length;};
  push('%PDF-1.4\n%âãÏÓ\n');const obj=(n:number,body:()=>void)=>{offsets[n]=len;push(`${n} 0 obj\n`);body();push('\nendobj\n');};
  obj(1,()=>push('<< /Type /Catalog /Pages 2 0 R >>'));obj(2,()=>push('<< /Type /Pages /Kids [3 0 R] /Count 1 >>'));obj(3,()=>push(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${pageW} ${pageH}] /Resources << /XObject << /Im0 4 0 R >> >> /Contents 5 0 R >>`));
