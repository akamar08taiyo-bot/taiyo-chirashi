export function buildPngPrintDocument(imageUrl:string,orientation:'portrait'|'landscape',title='チラシ'):string{
...
function escapePrintHtml(value:string){return value.replace(/[&<>]/g,(ch)=>ch==='&'?'&amp;':ch==='<'?'&lt;':'&gt;');}
function escapePrintAttr(value:string){return escapePrintHtml(value).replace(/"/g,'&quot;');}
