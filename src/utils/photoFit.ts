import type { PhotoFitMode, PhotoTransform } from '../types.js';
...
export function calculateImagePlacement(
  ...
  const iw=Math.max(1,imageWidth); const ih=Math.max(1,imageHeight);
  const bw=Math.max(1,boxWidth); const bh=Math.max(1,boxHeight);
  ...
  const width=iw*scale; const height=ih*scale;
  ...
  const x=Math.max(0,Math.min(100,transform.x)); const y=Math.max(0,Math.min(100,transform.y));
