...
export function validateImageFile(file: File): void {
  if (file.size > MAX_IMAGE_BYTES) throw new AppError('写真の容量が大きすぎます。20MB以下の写真を選んでください。');
  if (!allowedTypes.has(file.type)) {
    if (/hei[cf]/i.test(file.type) || /\.hei[cf]$/i.test(file.name)) throw new AppError('HEIC/HEIF形式はこのブラウザでは安定して編集できません。JPEG、PNG、WebP形式の写真を選んでください。');
    throw new AppError('対応していない写真形式です。JPEG、PNG、WebP形式の写真を選んでください。');
  }
}

export async function createPreviewBlob(file: Blob, maxDimension = 1600): Promise<Blob> {
  try {
    ...
    if (!ctx) throw new AppError('写真を処理できませんでした。別の写真をお試しください。');
...
export async function loadImage(source: string | Blob): Promise<HTMLImageElement> {
  ...
  try {
    const image = new Image(); image.decoding = 'async'; image.crossOrigin = source instanceof Blob ? '' : 'anonymous';
...
async function decodeImage(blob: Blob): Promise<{ width:number; height:number; source: CanvasImageSource; cleanup:()=>void }> {
