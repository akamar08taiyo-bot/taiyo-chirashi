import type { EditorState, FlyerMode } from '../types.js';

function inferMode(state: Partial<EditorState>): FlyerMode {
  const current = state.mode;
  if (current === 'rental' || current === 'cases' || current === 'consumables') return current;
  const title = state.title ?? '';
  const eyebrow = state.eyebrow ?? '';
  if (/消耗品|衛生用品|施設用品/.test(`${title}${eyebrow}`)) return 'consumables';
  if (/レンタル/.test(`${title}${eyebrow}`)) return 'rental';
  return 'cases';
}
 ...
 */
export function sanitizeEditorStateForServer(state: EditorState): EditorState {
