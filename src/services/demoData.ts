import type { AppContext, Category, EditorState, FlyerItem, FlyerMode, FlyerRecord, MediaRecord, Office, Organization, Profile, TemplateRecord } from '../types.js';
...
export function emptyItem(number: number): FlyerItem {
  return {
...
export function createDefaultEditorState(layoutCount: 1|2|3|4|6|9 = 9, mode: FlyerMode = 'cases'): EditorState {
  ...
  if (mode === 'rental') {
    ...
    state.eyebrow = 'レンタル'; state.eyebrowNote = '必要な福祉用具を分かりやすくご紹介';
  } else if (mode === 'consumables') {
    state.title = '施設向け 消耗品のご案内';
    state.subtitle = '日々の業務で使用する消耗品を、写真・規格・価格と一緒に分かりやすくご案内します。';
    state.eyebrow = '消耗品'; state.eyebrowNote = '施設・事業所向け商品';
    state.footerHeadline = '必要な商品・数量・お見積りについて、お気軽にお問い合わせください。';
    state.footerNote = '掲載価格・規格・入数は作成時点の情報です。詳細は担当者までお問い合わせください。';
    state.display.showUnits = false; state.display.showBurden1 = false; state.display.showBurden2 = false; state.display.showBurden3 = false; state.display.showPrices = true;
...
export function createDemoEditorState(): EditorState {
...
export function createDemoContext(): AppContext {
  return { organization: demoOrganization, offices: demoOffices, profiles: demoProfiles, categories: demoCategories };
}

/**
 * 花王「手指衛生」価格マスター（仕入・販売価格マスター.xlsx）の上位16商品。
 * 価格が入っている商品のみを掲載する消耗品チラシの元データ。
 * priceYenは販売単価（＝販売金額÷ケース入数）を円未満四捨五入した値。
 */
interface KaoHandHygieneProduct { code: string; name: string; spec: string; packSize: string; priceYen: number; type: string; }
const KAO_HAND_HYGIENE_PRODUCTS: KaoHandHygieneProduct[] = [
...
function kaoHandHygieneItem(number: number, product: KaoHandHygieneProduct): FlyerItem {
  return {
...
function kaoHandHygieneEditorState(products: KaoHandHygieneProduct[]): EditorState {
  ...
  state.eyebrow = '花王'; state.eyebrowNote = '手指衛生・消毒';
...
function createKaoHandHygieneFlyers(now: Date): FlyerRecord[] {
  const pages = chunk(KAO_HAND_HYGIENE_PRODUCTS, 4);
  const make = (id: string, title: string, pageProducts: KaoHandHygieneProduct[], minutesAgo: number): FlyerRecord => {
...
export function createSeedFlyers(): FlyerRecord[] {
  const now = new Date();
  const state = createDemoEditorState();
  const make = (id: string, title: string, ownerId: string, officeId: string, shareScope: 'private'|'office'|'company', minutesAgo: number): FlyerRecord => ({
...
export function createSeedTemplates(): TemplateRecord[] {
  ...
  ['1モーター', '2モーター', '3モーター'].forEach((motor, i) => {
    const item = specialBed.items[i]; if (!item) return;
  ...
  ['自走式', '介助式'].forEach((kind, i) => {
    const item = privateWheelchair.items[i]; if (!item) return;
...
export function createSeedMedia(): MediaRecord[] {
